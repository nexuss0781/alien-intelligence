...
// ---------------------------------------------------------------------------
__global__ void ssog_forward_kernel(
    ...
    int n_experts, int k_experts)
{
    ...
    float* s_hidden = s;
    float* s_weighted = &s_hidden[d_model];
    float* s_logits = &s_weighted[d_model];

    int j = threadIdx.x;

    if (j < d_model) s_hidden[j] = hidden[pos * d_model + j];
    if (j < d_model) s_weighted[j] = 0.0f;
    __syncthreads();
    ...
    for (int k = 0; k < k_experts; ++k) {
        ...
        if (j < d_model) {
            ...
            for (int l = 0; l < d_model; ++l)
                sum += W_expert[e * d_model * d_model + j * d_model + l] * s_hidden[l];
            sum += b_expert[e * d_model + j];
            s_weighted[j] += w * sum;
        }
        __syncthreads();
    }
    ...
    if (j < vocab) {
        float logit = b_out[j];
        for (int l = 0; l < d_model; ++l)
            logit += W_out[j * d_model + l] * s_weighted[l];
        s_logits[j] = logit;
    ...
    if (loss_out && j == 0) {
        ...
        for (int i = 1; i < vocab; ++i)
            if (s_logits[i] > mx) mx = s_logits[i];

        float sum_exp = 0.0f;
        for (int i = 0; i < vocab; ++i)
            sum_exp += expf(s_logits[i] - mx);

        int t = targets[pos];
        float p_target = expf((t >= 0 && t < vocab) ? s_logits[t] - mx : -mx);
...
float forward_backward(GPUContext* ctx,
                       ...
                       std::size_t vocab, std::size_t d_model)
{
    if (!ctx || n_positions <= 0 || vocab == 0 || d_model == 0) return 0.0f;
    ...
    CUDA_CHECK(cudaMemcpy(ctx->d_hidden, hidden, n * d * sizeof(float),
                           cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ctx->d_expert_idxs, expert_idxs, n * k * sizeof(int),
                           cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ctx->d_expert_wgts, expert_wgts, n * k * sizeof(float),
                           cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(ctx->d_targets, targets, n * sizeof(int),
                           cudaMemcpyHostToDevice));

    // Forward kernel
    int smem = (d + d + v) * sizeof(float);
    int threads = d > v ? d : v;

    ssog_forward_kernel<<<n, threads, smem>>>(
        ctx->d_hidden, ctx->d_expert_idxs, ctx->d_expert_wgts, ctx->d_targets,
        ctx->d_W_expert, ctx->d_b_expert,
        ctx->d_W_out, ctx->d_b_out,
        ctx->d_logits, ctx->d_loss,
        n, v, d, ne, k);
    ...
    batched_gradient_kernel<<<v, gthreads, 0>>>(
